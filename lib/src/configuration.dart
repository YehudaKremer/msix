...
class Configuration {
  ...
  String msixFontsPath = '';
  List<String>? msixFontsFiles;
  ...
  Future<void> getConfigValues() async {
    ...
    toastActivatorDisplayName =
        _args['toast-activator-display-name'] ?? toastActivatorYaml['display_name']?.toString() ?? 'Toast activator';
    ...
    hoursBetweenUpdateChecks =
        int.parse(_args['hours-between-update-checks'] ?? installerYaml['hours_between_update_checks']?.toString() ?? '0');
    ...
    automaticBackgroundTask = _args.wasParsed('automatic-background-task') ||
        installerYaml['automatic_background_task']?.toString().toLowerCase() == 'true';
    updateBlocksActivation = _args.wasParsed('update-blocks-activation') ||
        installerYaml['update_blocks_activation']?.toString().toLowerCase() == 'true';
    showPrompt = _args.wasParsed('show-prompt') || installerYaml['show_prompt']?.toString().toLowerCase() == 'true';
    forceUpdateFromAnyVersion = _args.wasParsed('force-update-from-any-version') ||
        installerYaml['force_update_from_any_version']?.toString().toLowerCase() == 'true';
    ...
    contextMenuConfiguration = contextMenuYaml != null && contextMenuYaml is YamlMap && !skipContextMenu
  ...
  Future<void> validateWindowsBuildFiles() async {
    ...
    if (!await Directory(buildFilesFolder).exists() ||
        !await Directory(buildFilesFolder).list().any((file) => file.path.endsWith('.exe'))) {
  ...
  Future<void> validateAppInstallerConfigValues() async {
    ...
    if (publishFolderPath.isNullOrEmpty ||
        (!await Directory(publishFolderPath!).exists() &&
            !await Directory(publishFolderPath = '${Directory.current.path}\\${publishFolderPath!}').exists())) {
  ...
  Future<void> _getMsixFonts(pubspec) async {
    dynamic yaml = pubspec['flutter'] ?? YamlMap();
    dynamic families = yaml["fonts"];

    if (families != null) {
      for (var family in families) {
        msixFontsFiles = [];
        for (var font in family["fonts"]) {
          if (font["asset"]?.startsWith("fonts/") ?? false) {
            msixFontsFiles!.add(font["asset"]!.replaceFirst("fonts/", ''));
          }
        }
      }
    }
  ...
  Iterable<String>? _getLanguages(dynamic config) => ((_args['languages'] ?? config['languages']) as String?)
      ?.split(',')
      .map((e) => e.trim())
  ...
  Iterable<String>? _getAppUriHandlerHosts(dynamic config) =>
      ((_args['app-uri-handler-hosts'] ?? config['app_uri_handler_hosts']) as String?)
          ?.split(',')
          .map((e) => e.trim())
          .where((element) => element.isNotEmpty);
